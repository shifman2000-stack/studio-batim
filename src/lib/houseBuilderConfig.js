...
/* DEFAULT_PROPS הוסר בכוונה.
   בעבר חלל שלא הופיע ב-ROOM_PROPS קיבל רשימת מאפיינים גנרית
   ("דורש חלון / קרבה לחלל הציבורי / דגש מיוחד"). זה הפך "אפס מאפיינים"
   ללא-ניתן-להגדרה: רשימה ריקה ב-house_builder_config נקראה כ"לא הוגדר"
   וירשה את ברירת המחדל. היום המאפיינים מגיעים אך ורק מהקונפיג — סוג
   חלל בלי קבוצות מאפיינים פשוט לא מציג מקטע מאפיינים כלל. */
