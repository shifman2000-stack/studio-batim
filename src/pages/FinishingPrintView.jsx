// src/pages/FinishingPrintView.jsx
// Public, standalone route used by Puppeteer to render the finishing-materials
// report as a clean A4 document. No nav, no auth wrapper — just the report.
//
// Mirrors QuotePrintView's structure exactly.

import { useEffect, useState } from 'react'
import { useParams } from 'react-router-dom'
import { supabase } from '../supabaseClient'
import FinishingReport from '../components/FinishingReport'

export default function FinishingPrintView() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Bypasses RLS on projects + project_contacts + client_info + project_finishing_materials
      // so Puppeteer (unauthenticated) can render the PDF.
      const { data: rpcData, error } = await supabase
        .rpc('get_finishing_report_for_print', { p_project_id: projectId })
