// src/pages/QuantitiesPrintView.jsx
// Public, standalone route used by Puppeteer to render the quantities
...
import QuantitiesReport from '../components/QuantitiesReport'

export default function QuantitiesPrintView() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: rpcData, error } = await supabase
        .rpc('get_quantities_report_for_print', { p_project_id: projectId })
  ...
  if (error) {
    return (
      <div style={{ padding: 24, fontFamily: 'Heebo, sans-serif', direction: 'rtl', color: '#c0392b' }}>
        שגיאה בטעינת כתב הכמויות: {error}
      </div>
    )
  }
  ...
  return (
    <div className="quantities-print-mode" data-quantities-ready="true">
      <style>{`
        /* Force A4 paged layout for Puppeteer. Margins kept in sync with QuantitiesReport.css. */
        @page {
          size: A4 portrait;
          margin: 15mm 12mm;
        }
